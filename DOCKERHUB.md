...
```
docker pull amslertec/pwnotify:0.3.4
```
...
- **Modern UI** — dashboard, users table with CSV/XLSX export, notification & run history,
  editable DE/EN templates, branding, local + Microsoft-SSO logins, profile photos across
  user tables (Entra + local/SSO).
- **Multi-tenant** — manage one Entra tenant or many customer tenants from a single
  instance, off by default (Settings → General); per-customer isolation via PostgreSQL
  Row-Level Security under a restricted DB role.
- **Teams & access model** — global superadmin, plus local admins/auditors scoped to
  customers via grants or Entra-group-based Teams, with Graph member sync driving access
  automatically.
...
| Tag | Meaning |
|---|---|
| `0.3.4` | Pinned release (recommended for production) |
...
```bash
docker scout cves amslertec/pwnotify:0.3.4            # vulnerability report
docker buildx imagetools inspect amslertec/pwnotify:0.3.4   # platforms + attestations
docker scout sbom amslertec/pwnotify:0.3.4            # software bill of materials
```
