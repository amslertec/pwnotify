...
export function TwoFactorSection() {
  ...
  return (
    <div className="border-border bg-card flex h-full flex-col rounded-xl border p-5">
      {/* Kopf: Schild + Status. Farbe kodiert den Zustand auf einen Blick. */}
      <div className="flex items-start gap-3">
        ...
        >
          {active ? (
            <ShieldCheck className="size-5" style={{ color: 'var(--status-ok)' }} />
          ) : (
            <ShieldAlert className="text-muted-foreground size-5" />
          )}
        </div>
        <div className="min-w-0">
          <h3 className="font-display text-sm font-semibold">{t('twofa.title')}</h3>
          <p className="text-muted-foreground mt-0.5 text-sm">
            {active ? t('twofa.active') : t('twofa.description')}
          </p>
        </div>
      </div>
