"""H1: an over-long `User-Agent` header must not disable the account lockout or the audit
trail.

`audit_log.user_agent` and `user_session.user_agent` are both `varchar(400)`. Postgres does
NOT truncate an over-long value -- it REJECTS the INSERT with `value too long for type
character varying(400)`. In the login handler the failed-attempt counter and the
`LOGIN_FAILED`/`ACCOUNT_LOCKED` audit rows are persisted in ONE shared `session.commit()`,
so a 500-char header failed the audit INSERT, rolled the whole transaction back, and left
`failed_login_count` un-persisted -- the lockout after `login_max_failures` never fired and
no audit entry was written. That turns the fail-closed coupling into a brute-force enabler.
...
each attempt runs on its own `get_session_factory()` session exactly like a real request.
The `@limiter.limit` decorator is disabled (duck-typed request), same pattern as
`test_login_enumeration.py` / `test_switch_tenant.py`.
"""
...
# 500 chars > the varchar(400) column width -- long enough that Postgres rejects the INSERT.
LONG_UA = "UA-" + "x" * 500
...
def _uname(label: str) -> str:
    return f"h1-{label}-{uuid.uuid4().hex[:10]}"
...
async def test_password_login_lockout_survives_overlong_user_agent(
    migrated_engine: AsyncEngine,
) -> None:
    """Five wrong-password attempts with a 500-char UA must still lock the account and leave
    five `LOGIN_FAILED` audit entries. Before the fix the shared commit rolled back on the
    varchar rejection, so neither the lock nor any audit row was ever persisted (RED)."""
    async with get_session_factory()() as s:
    ...
    try:
        for _ in range(5):
            async with get_session_factory()() as s:
                ...
                # New code: raises AuthError(invalid_credentials). Old code: raises the asyncpg
                # varchar-rejection at commit. Either way the persisted state is what matters.
        ...
        async with get_session_factory()() as s:
            ...
            assert locked.locked_until is not None, (
                "account was never locked -- the over-long UA rolled back the shared commit, so "
...
async def test_2fa_lockout_survives_overlong_user_agent(migrated_engine: AsyncEngine) -> None:
    """Five wrong 2FA codes with a 500-char UA must lock the account AND leave the audit
    entries. The 2FA path commits the counter separately (so the lock itself survived), but
    the `LOGIN_FAILED`/`ACCOUNT_LOCKED` audit rows shared the later commit that the varchar
    rejection rolled back -- zero audit rows before the fix (RED)."""
    async with get_session_factory()() as s:
        user = await user_repo.create(
    ...
    try:
        for _ in range(5):
            async with get_session_factory()() as s:
                request = _FakeRequest(user_agent=LONG_UA, cookies={TWOFA_COOKIE: twofa_cookie})
        ...
        async with get_session_factory()() as s:
            ...
            assert failed == 5, (
                ...
                "over-long UA rolled back the audit commit (RED before the fix)"
            )
            assert await _count_audit(s, user_id, "auth.account_locked") == 1
    finally:
        await _cleanup(user_id)
...
async def test_complete_login_survives_overlong_user_agent(
    migrated_engine: AsyncEngine, session: AsyncSession
) -> None:
    """A successful login carrying a 500-char UA must succeed and create a `user_session`
    row whose `user_agent` fits the varchar(400) column. Before the fix `_complete_login`'s
    ...
    assert us.user_agent is not None
    assert len(us.user_agent) <= 400, (
        f"user_session.user_agent was not truncated to the column width: {len(us.user_agent)}"
    )
